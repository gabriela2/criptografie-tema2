...
from BlumMicaliGenerator import BlumMicaliGenerator
...
class BlumMicaliGeneratorApplication(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        self.bit_length_frame = tk.LabelFrame(self.parameters_frame, text="Size of the output file (MB)")
        self.bit_length_frame.pack(fill='x', expand=True)
        self.bit_length_input = tk.Entry(self.bit_length_frame, bd=5)
        self.bit_length_input.insert(tk.END, 1)
        ...
        self.statistics_frame = tk.LabelFrame(text="Statistics")
    ...
    def _generate_key(self):
        try:
            ...
            messagebox.showinfo("Initialization", "There are input fields that are null")
            return
        bit_length: float = float(self.bit_length_input.get())*1048576
        start = time.time()
